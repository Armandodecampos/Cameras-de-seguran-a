...
class CentralMonitoramento(ctk.CTk):
    ...
    def __init__(self):
        ...
        for i in range(self.GRID_COLS): self.grid_frame.grid_rowconfigure(i, weight=1)
    ...
    def maximizar_slot(self, index):
        ...
        for i in range(self.MAX_SLOTS):
            frm = self.slot_frames[i]
            if i == index:
                frm.grid_configure(row=0, column=0, rowspan=self.GRID_COLS, columnspan=self.GRID_COLS, padx=0, pady=0, sticky="nsew")
                frm.configure(corner_radius=0)
    ...
    def _pos_conexao(self, sucesso, camera_obj, ip, erro=None):
        if sucesso:
            ...
            if ip not in self.grid_cameras:
                ...
                if camera_obj: camera_obj.parar()
    ...
    def atualizar_lista_presets_ui(self):
        ...
        for nome in presets:
            ...
            # Bind para o frame e para o label
            for widget in [frm, lbl]:
