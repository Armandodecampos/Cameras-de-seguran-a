...
class CentralMonitoramento(ctk.CTk):
    ...
    def __init__(self):
        ...
        self.btn_save_grid = ctk.CTkButton(self.frame_controles, text="Salvar Grid 💾",
                                            fg_color=self.ACCENT_WINE, hover_color=self.ACCENT_RED,
                                            command=lambda: self.salvar_preset_atual())
    ...
    def ao_arrastar_divisor(self, event):
        ...
        if 200 < nova_largura < 800:
            ...
            for lbl_preset in self.preset_labels_referencia.values():
                try: lbl_preset.configure(wraplength=nova_largura - 120)
                except: pass
    ...
    def salvar_preset_atual(self, nome=None):
        if not nome:
            self.abrir_modal_input("Salvar Predefinição", "Digite o nome da predefinição:", self.confirmar_salvamento_preset)
        else:
    ...
    def aplicar_preset(self, nome):
        if nome in self.presets_salvos:
            ...
            # Garante que a lista tenha MAX_SLOTS elementos
            full_list = ["0.0.0.0"] * self.MAX_SLOTS
            for i in range(min(len(lista_ips), self.MAX_SLOTS)):
    ...
    def atualizar_lista_presets_ui(self):
        ...
        for nome in presets:
            ...
            btn_del = ctk.CTkButton(frm, text="🗑️", width=30, height=30, fg_color=self.GRAY_DARK,
            ...
            lbl = ctk.CTkLabel(frm, text=nome, font=("Roboto", 14), anchor="w", justify="left",
                               wraplength=max(50, self.sidebar.winfo_width() - 120))
