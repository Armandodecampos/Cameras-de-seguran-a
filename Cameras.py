    ...
    def loop_leitura(self):
        ...
        while self.rodando:
            ...
            if ret:
                ...
                if consecutive_failures > 300: # Aprox 3 segundos (0.01s sleep no loop)
...
class CentralMonitoramento(ctk.CTk):
    ...
    def __init__(self):
        ...
        self.arquivo_predefinicoes = os.path.join(user_dir, "presets_grid_abi.json")
        ...
        # Aplica automaticamente o última predefinição se existir
    ...
    def atribuir_ip_ao_slot(self, idx, ip, atualizar_ui=True, gerenciar_conexoes=True, salvar=True, forcado=False):
        ...
        # Otimização: se o IP for o mesmo, não faz nada (a menos que seja reset 0.0.0.0 ou forçado)
        if not forcado and ip == self.grid_cameras[idx] and ip != "0.0.0.0":
            return
        ...
        try:
            # Tenta configurar o label existente (imagem e texto independentes para robustez)
            try:
                self.slot_labels[idx].configure(image=self.img_vazia)
    ...
    def aplicar_predefinicao(self, nome):
        ...
        for i in range(20):
            target_ip = predefinicao[i] if i < len(predefinicao) else "0.0.0.0"
            novos_ips[i] = target_ip

            # Otimização: Só atualiza slots que realmente mudaram de IP
            if self.grid_cameras[i] != target_ip:
                self.atribuir_ip_ao_slot(i, target_ip, atualizar_ui=False, gerenciar_conexoes=False, salvar=False)
    ...
    def atualizar_lista_predefinicoes_ui(self):
        ...
        for nome in sorted(self.predefinicoes.keys()):
            ...
            # Label de Nome (Ocupa o resto do espaço à esquerda)
            ...
            # Binds de clique para aplicar a predefinição
