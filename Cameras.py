...
class CentralMonitoramento(ctk.CTk):
    ...
    def ao_soltar_slot(self, event, index):
        ...
        try:
            ...
            if 0 <= source_idx < 20 and 0 <= target_idx < 20:
                ...
                # Usamos atribuir_ip_ao_slot para garantir que toda a lógica de conexões
                # e limpeza de UI seja executada. Usamos forcado=True para ignorar
                # a otimização de "mesmo IP" durante o swap.
                self.atribuir_ip_ao_slot(source_idx, ip_tgt, atualizar_ui=False, forcado=True)
                self.atribuir_ip_ao_slot(target_idx, ip_src, atualizar_ui=False, forcado=True)
    ...
    def atribuir_ip_ao_slot(self, idx, ip, atualizar_ui=True, gerenciar_conexoes=True, salvar=True, forcado=False):
        ...
        # Se for o mesmo IP e não for forçado, ignora (exceto para limpar slot)
        if not forcado and ip == ip_antigo and ip != "0.0.0.0":
            return

        # Limpa predefinicao ao atribuir manualmente (se houver mudança real)
    ...
    def sobrescrever_predefinicao(self, nome):
        self.abrir_modal_confirmacao("Confirmar", f"Deseja sobrescrevê-la ('{nome}') com a configuração atual?",
