...
class CentralMonitoramento(ctk.CTk):
    ...
    def atribuir_ip_ao_slot(self, idx, ip, atualizar_ui=True, gerenciar_conexoes=True, salvar=True, forcado=False):
        ...
        # Otimização: se o IP for o mesmo, pula (a menos que seja forçado ou o IP 0.0.0.0 que pode precisar resetar texto)
    ...
    def aplicar_predefinicao(self, nome):
        ...
        # 2. Atualiza os dados do grid primeiro (silenciosamente e sem salvar a cada passo)
        novos_ips = ["0.0.0.0"] * 20
        for i in range(20):
            ip = predefinicao[i] if i < len(predefinicao) else "0.0.0.0"
            novos_ips[i] = ip

            # Atualiza visualmente cada slot de forma segura, evitando o IP-matching lock para forçar refresh
    ...
    def atualizar_lista_predefinicoes_ui(self):
        ...
        for nome in sorted(self.predefinicoes.keys()):
            ...
            # Botões de Controle (direita para esquerda)
