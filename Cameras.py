    ...
    def loop_leitura(self):
        ...
        while self.rodando:
            ...
            if ret:
                ...
                # Controle de FPS Dinâmico (Reduzido para 5 em background para economizar CPU/Rede mas manter fluidez)
...
class CentralMonitoramento(ctk.CTk):
    ...
    def confirmar_salvamento_preset(self, nome):
        if nome in self.presets_salvos:
            self.abrir_modal_alerta("Aviso", f"Já existe uma predefinição com o nome '{nome}'.")
            return
    ...
    def _pos_conexao(self, sucesso, camera_obj, ip, erro=None):
        if sucesso:
            ...
            # Verifica se o IP ainda é necessário
            if ip not in self.grid_cameras:
                camera_obj.parar()
                return
