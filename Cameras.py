...
class CentralMonitoramento(ctk.CTk):
    ...
    def ao_soltar_slot(self, event, index):
        ...
        try:
            ...
            if 0 <= source_idx < 20 and 0 <= target_idx < 20:
                ...
                # Realiza a troca usando atribuir_ip_ao_slot para garantir sincronia
                # Note: atribuir_ip_ao_slot já lida com a limpeza da 'ultima_predefinicao'
    ...
    def atribuir_ip_ao_slot(self, idx, ip, atualizar_ui=True, gerenciar_conexoes=True, salvar=True, forcado=False):
        ...
        # Otimização: Se o IP já for o mesmo, não faz nada (a menos que seja reset 0.0.0.0 ou forçado)
    ...
    def aplicar_predefinicao(self, nome):
        predefinição = self.predefinicoes.get(nome)
        if not predefinição: return

        # Limpa o cooldown para permitir reconexão imediata se for um predefinição
        ...
        for i in range(20):
            ip = predefinição[i] if i < len(predefinição) else "0.0.0.0"
            novos_ips[i] = ip

            # Atualiza visualmente cada slot de forma segura
            # Usamos salvar=False para fazer um único save no final, e forcado=True para garantir que o grid reflita o preset
        ...
        # 5. Restaura layout se necessário, salva o grid e seleciona o slot
        if self.slot_maximized is not None:
            self.restaurar_grid()
    ...
    def _sobrescrever_predefinicao(self, nome):
        self.predefinicoes[nome] = list(self.grid_cameras)
        self.ultima_predefinicao = nome # Mantém a seleção visual
        self.salvar_predefinicoes()
        self.atualizar_lista_predefinicoes_ui()
    ...
    def renomear_predefinicao(self, nome_antigo):
        def on_name_entered(novo_nome):
            ...
            if not novo_nome:
                self.abrir_modal_alerta("Aviso", "O nome não pode ser vazio.")
                return
            ...
            if novo_nome != nome_antigo:
                ...
                # Garante que o nome antigo ainda existe (segurança contra race conditions)
    ...
    def atualizar_lista_predefinicoes_ui(self):
        ...
        for nome in sorted(self.predefinicoes.keys()):
            ...
            # Botoes de Acao (Direita para Esquerda: X, Renomear, Salvar)
            btn_del = ctk.CTkButton(frm, text="X", width=30, height=30, fg_color=self.ACCENT_WINE,
                                     hover_color=self.ACCENT_RED, command=lambda n=nome: self.deletar_predefinicao(n))
            ...
            btn_save = ctk.CTkButton(frm, text="💾", width=30, height=30, fg_color=self.GRAY_DARK,
                                     hover_color=self.ACCENT_RED, command=lambda n=nome: self.sobrescrever_predefinicao(n))
            btn_save.pack(side="right", padx=2)

            # Label (Preenche o resto)
