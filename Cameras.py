...
class CentralMonitoramento(ctk.CTk):
    ...
    def atribuir_ip_ao_slot(self, idx, ip, atualizar_ui=True, gerenciar_conexoes=True, salvar=True, forcado=False):
        ...
        # Otimização: se for o mesmo IP já presente no slot, não faz nada (a menos que seja forçado ou 0.0.0.0)
        ...
        try:
            ...
            print(f"Erro visual ao configurar imagem/texto no slot {idx}: {e}")

        try:
            # Limpa cache do slot para evitar fantasmas ou falhas de sincronia
            self.slot_ctk_images[idx] = None
        except Exception as e:
            print(f"Erro ao limpar cache de imagem no slot {idx}: {e}")

        # Se houver falha crítica, tenta recriar o label
        if self.slot_labels[idx] is None:
    ...
    def aplicar_predefinicao(self, nome):
        ...
        for i in range(20):
            ...
            # Atualiza visualmente cada slot de forma segura (forcado=True e salvar=False)
    ...
    def atualizar_lista_predefinicoes_ui(self):
        ...
        for nome in sorted(self.predefinicoes.keys()):
            ...
            # Botões de Controle (Da Direita para Esquerda: X, ✎, 💾)
            btn_del = ctk.CTkButton(frm, text="X", width=30, height=30, fg_color="transparent",
                                     text_color=self.TEXT_S, hover_color=self.ACCENT_RED,
            ...
            # Label de Nome (Expande para preencher o resto)
