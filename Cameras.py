...
class CentralMonitoramento(ctk.CTk):
    ...
    def carregar_predefinicoes(self):
        # 1. Tenta carregar do novo arquivo
        ...
        # 2. Migração: Tenta carregar do arquivo antigo se o novo não existir
        ...
        if os.path.exists(arquivo_antigo):
            try:
                with open(arquivo_antigo, "r", encoding='utf-8') as f:
                    dados = json.load(f)
                    if dados:
                        # Salva imediatamente no novo formato/arquivo
                        self.predefinicoes = dados
    ...
    def atualizar_lista_predefinicoes_ui(self):
        ...
        for nome in sorted(self.predefinicoes.keys(), key=str.lower):
            ...
            btn_ren.pack(side="right", padx=0)
            ...
            btn_sav.pack(side="right", padx=0)
