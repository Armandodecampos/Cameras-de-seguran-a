...
class CentralMonitoramento(ctk.CTk):
    ...
    def __init__(self):
        ...
        tab_predefinicoes.grid_columnconfigure(0, weight=1)
    ...
    def atribuir_ip_ao_slot(self, idx, ip, atualizar_ui=True, gerenciar_conexoes=True):
        ...
        # Note: 'aplicar_predefinicao' chama atribuir_ip_ao_slot with gerenciar_conexoes=False
    ...
    def carregar_predefinicoes(self):
        # Implementa uma migração única: se o novo arquivo não existe, tenta carregar o antigo
        if not os.path.exists(self.arquivo_predefinicoes):
            user_dir = os.path.expanduser("~")
            arquivo_antigo = os.path.join(user_dir, "presets_grid_abi.json")
            if os.path.exists(arquivo_antigo):
                try:
                    with open(arquivo_antigo, "r", encoding='utf-8') as f:
                        dados = json.load(f)
                        # Salva imediatamente no novo local para completar a migração
                        with open(self.arquivo_predefinicoes, "w", encoding='utf-8') as fn:
                            json.dump(dados, fn, ensure_ascii=False, indent=4)
                        return dados
                except: pass
    ...
    def deletar_predefinicao(self, nome):
        self.abrir_modal_confirmacao("Confirmar", f"Deseja realmente excluí-la (a predefinição '{nome}')?",
    ...
    def atualizar_lista_predefinicoes_ui(self):
        ...
        for nome in sorted(self.predefinicoes.keys()):
            ...
            # Botão de Salvar [💾] para sobrescrever
            ...
            # Bind no Frame E no Label para facilitar o clique de aplicação
