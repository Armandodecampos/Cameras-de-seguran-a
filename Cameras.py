...
class CentralMonitoramento(ctk.CTk):
    ...
    def atribuir_ip_ao_slot(self, idx, ip, atualizar_ui=True, gerenciar_conexoes=True, salvar=True, forcado=False):
        ...
        # Otimização: se for o mesmo IP já presente no slot, não faz nada
        # (A menos que seja "0.0.0.0" ou uma atualização forçada)
        if not forcado and ip != "0.0.0.0" and ip == self.grid_cameras[idx]:
            return
    ...
    def aplicar_predefinicao(self, nome):
        ...
        # 2. Atualiza os dados do grid primeiro (silenciosamente e otimizado)
        novos_ips = ["0.0.0.0"] * 20
        for i in range(20):
            ip = predefinicao[i] if i < len(predefinicao) else "0.0.0.0"
            novos_ips[i] = ip

            # Otimização: Forçamos a atualização apenas se for um IP diferente ou for o primeiro carregamento
        ...
        # 5. Salva estado final e atualiza interface
        self.salvar_grid()
        if self.slot_maximized is not None:
            self.restaurar_grid()
    ...
    def sobrescrever_predefinicao(self, nome):
        self.abrir_modal_confirmacao("Confirmar", f"Deseja sobrescrevê-la (a predefinição '{nome}') com a configuração atual?",
    ...
    def atualizar_lista_predefinicoes_ui(self):
        ...
        for nome in sorted(self.predefinicoes.keys()):
            ...
            # Botões de Controle (X, ✎, 💾) - Ordem de pack side='right' (X, ✎, 💾)
            # Ficam visualmente como [💾] [✎] [X] da esquerda para a direita no lado direito.
