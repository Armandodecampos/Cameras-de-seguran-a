...
class CentralMonitoramento(ctk.CTk):
    ...
    def __init__(self):
        ...
        if self.ultimo_predefinicao and self.ultimo_predefinicao in self.predefinicoes:
            self.after(500, lambda: self.aplicar_predefinicao(self.ultimo_predefinicao))
        ...
        if os.path.exists(self.arquivo_janela):
            try:
                with open(self.arquivo_janela, "r") as f:
                    ...
                    self.ultimo_predefinicao = dados.get("last_predefinicao")
        ...
        try:
            if not self.em_tela_cheia:
                dados = {
                    ...
                    "last_predefinicao": self.ultimo_predefinicao,
    ...
    def ao_soltar_slot(self, event, index):
        ...
        try:
            ...
            if 0 <= source_idx < 20 and 0 <= target_idx < 20:
                ...
                if self.ultimo_predefinicao:
                    self.pintar_predefinicao(self.ultimo_predefinicao, self.BG_SIDEBAR)
                    self.ultimo_predefinicao = None
    ...
    def atribuir_ip_ao_slot(self, idx, ip, atualizar_ui=True, gerenciar_conexoes=True, salvar=True, forcado=False):
        ...
        # Otimização: se o IP já for o mesmo, ignora (a menos que seja reset ou forçado)
        ...
        if gerenciar_conexoes and self.ultimo_predefinicao:
            self.pintar_predefinicao(self.ultimo_predefinicao, self.BG_SIDEBAR)
            self.ultimo_predefinicao = None
    ...
    def carregar_predefinicoes(self):
        ...
        # Migração de legado: presets_grid_abi.json -> predefinicoes_grid_abi.json
        arquivo_legado = os.path.join(os.path.expanduser("~"), "presets_grid_abi.json")
        if os.path.exists(arquivo_legado):
            try:
                with open(arquivo_legado, "r", encoding='utf-8') as f:
                    dados = json.load(f)
                if dados:
                    # Salva no novo local para completar a migração
    ...
    def aplicar_predefinicao(self, nome):
        ...
        # 2. Atualiza os dados do grid e UI de forma otimizada
        novos_ips = ["0.0.0.0"] * 20
        for i in range(20):
            ip = predefinicao[i] if i < len(predefinicao) else "0.0.0.0"
            novos_ips[i] = ip

            # Atualiza apenas se for necessário (IP mudou)
            # atribuir_ip_ao_slot já tem a otimização de IP igual
        ...
        # 4. Inicia conexões para os novos IPs
    ...
    def _deletar_predefinicao(self, nome):
        if nome in self.predefinicoes:
            ...
            if self.ultimo_predefinicao == nome:
                self.ultimo_predefinicao = None
            self.salvar_predefinicoes()
            self.atualizar_lista_predefinicoes_ui()
        ...
        def on_name_entered(novo_nome):
            ...
            if novo_nome and novo_nome != nome_antigo:
                ...
                if nome_antigo in self.predefinicoes:
                    ...
                    if self.ultimo_predefinicao == nome_antigo:
                        self.ultimo_predefinicao = novo_nome
                    self.salvar_predefinicoes()
                    self.atualizar_lista_predefinicoes_ui()
    ...
    def atualizar_lista_predefinicoes_ui(self):
        ...
        for nome in sorted(self.predefinicoes.keys()):
            cor = self.ACCENT_WINE if nome == self.ultimo_predefinicao else self.BG_SIDEBAR
            ...
            # Botões de controle à direita (Pack em ordem reversa: X, ✎, 💾)
            btn_del = ctk.CTkButton(frm, text="X", width=30, height=30, fg_color="transparent",
                                     text_color=self.TEXT_S, hover_color=self.ACCENT_RED,
            ...
            btn_sav = ctk.CTkButton(frm, text="💾", width=30, height=30, fg_color="transparent",
                                     text_color=self.TEXT_S, hover_color=self.GRAY_DARK,
                                     command=lambda n=nome: self.sobrescrever_predefinicao(n))
            btn_sav.pack(side="right", padx=2)
            ...
            # Binds para aplicar predefinição
            for w in [frm, lbl]:
                w.bind("<Button-1>", lambda e, n=nome: self.aplicar_predefinicao(n))
