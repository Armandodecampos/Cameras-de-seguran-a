...
class CentralMonitoramento(ctk.CTk):
    ...
    def atribuir_ip_ao_slot(self, idx, ip, atualizar_ui=True, gerenciar_conexoes=True, salvar=True, forcado=False):
        ...
        # Se o IP já estiver no slot e não for uma atualização forçada, ignora (exceto se for 0.0.0.0 para limpar)
    ...
    def aplicar_predefinicao(self, nome):
        ...
        # 2. Atualiza os dados do grid primeiro (silenciosamente e em lote)
        novos_ips = ["0.0.0.0"] * 20
        for i in range(20):
            ip = predefinicao[i] if i < len(predefinicao) else "0.0.0.0"
            novos_ips[i] = ip

            # Atualiza visualmente cada slot de forma segura, sem salvar individualmente
        ...
        # 5. Salva o novo grid uma única vez
        self.salvar_grid()

        # 6. Restaura layout se necessário e seleciona slot
        if self.slot_maximized is not None:
            self.restaurar_grid()
    ...
    def atualizar_lista_predefinicoes_ui(self):
        ...
        # Reconstroi a lista alfabeticamente
        for nome in sorted(self.predefinicoes.keys()):
            ...
            # Botões de controle à direita (ordem inversa de packing para manter X na ponta)
            ...
            # Binds para aplicar a predefinição
