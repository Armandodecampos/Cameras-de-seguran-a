...
class CentralMonitoramento(ctk.CTk):
        ...
        if os.path.exists(self.arquivo_janela):
            try:
                with open(self.arquivo_janela, "r") as f:
                    ...
                    # Suporte para compatibilidade com o nome antigo
    ...
    def atribuir_ip_ao_slot(self, idx, ip, atualizar_ui=True, gerenciar_conexoes=True, salvar=True, forcado=False):
        ...
        # Se já for o mesmo IP, não faz nada (a menos que seja o reset 0.0.0.0 ou forçado)
    ...
    def carregar_predefinicoes(self):
        ...
        # Migração de dados do nome antigo (presets_grid_abi.json)
        arquivo_antigo = os.path.join(os.path.expanduser("~"), "presets_grid_abi.json")
        if os.path.exists(arquivo_antigo):
            try:
                with open(arquivo_antigo, "r", encoding='utf-8') as f:
                    dados = json.load(f)
                    # Salva no novo local imediatamente para completar a migração
    ...
    def aplicar_predefinicao(self, nome):
        ...
        # 2. Atualiza os dados do grid primeiro (silenciosamente e sem salvar para cada slot)
        novos_ips = ["0.0.0.0"] * 20
        for i in range(20):
            ip = predefinicao[i] if i < len(predefinicao) else "0.0.0.0"
            novos_ips[i] = ip

            # Atualiza visualmente cada slot de forma segura, mas não salva individualmente
            self.atribuir_ip_ao_slot(i, ip, atualizar_ui=False, gerenciar_conexoes=False, salvar=False)

        # Salva o grid uma única vez após a atualização em lote
        self.salvar_grid()
    ...
    def atualizar_lista_predefinicoes_ui(self):
        ...
        for nome in sorted(self.predefinicoes.keys()):
            ...
            # Binds para aplicar predefinição ao clicar no frame ou no label
            for widget in [frm, lbl]:
                widget.bind("<Button-1>", lambda e, n=nome: self.aplicar_predefinicao(n))
                widget.configure(cursor="hand2")
