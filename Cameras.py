...
class CentralMonitoramento(ctk.CTk):
    ...
    def __init__(self):
        ...
        # Segmented Button para alternar abas
        self.seg_button = ctk.CTkSegmentedButton(self.sidebar, values=["Câmeras", "Presets"],
                                                  command=self.mudar_aba_sidebar,
                                                  fg_color=self.BG_PANEL, selected_color=self.ACCENT_RED,
                                                  unselected_color=self.BG_SIDEBAR, text_color=self.TEXT_P)
        self.seg_button.pack(fill="x", padx=10, pady=10)
        self.seg_button.set(self.aba_ativa)

        # Container para as abas
        self.sidebar_container = ctk.CTkFrame(self.sidebar, fg_color="transparent")
        self.sidebar_container.pack(expand=True, fill="both")

        # --- ABA CÂMERAS ---
        self.frame_cameras = ctk.CTkFrame(self.sidebar_container, fg_color="transparent")
        ...
        # --- ABA PRESETS ---
        self.frame_presets = ctk.CTkFrame(self.sidebar_container, fg_color="transparent")

        self.btn_save_preset = ctk.CTkButton(self.frame_presets, text="Salvar Preset Atual",
                                              fg_color=self.ACCENT_WINE, hover_color=self.ACCENT_RED,
                                              corner_radius=0, height=40, command=self.abrir_modal_salvar_preset)
    ...
    def maximizar_slot(self, index):
        ...
        for i, frm in enumerate(self.slot_frames):
            if i == index:
                frm.grid_configure(row=0, column=0, rowspan=1, columnspan=1, padx=0, pady=0, sticky="nsew")
                frm.configure(corner_radius=0)
    ...
    def mudar_aba_sidebar(self, aba):
        ...
        if aba == "Câmeras":
            ...
            self.frame_cameras.pack(expand=True, fill="both")
        else:
            self.frame_cameras.pack_forget()
            self.frame_presets.pack(expand=True, fill="both")
    ...
    def atualizar_lista_presets_ui(self):
        ...
        for nome in sorted(self.presets.keys()):
            ...
            btn_edit = ctk.CTkButton(frm, text="✎", width=30, height=30, fg_color="transparent",
                                      ...
                                      command=lambda x=nome: self.renomear_preset_modal(x))
    ...
    def abrir_modal_input(self, titulo, mensagem, callback, valor_inicial=""):
        ...
        ctk.CTkLabel(modal, text=mensagem, font=("Roboto", 14), text_color=self.TEXT_P, wraplength=350).pack(pady=(20, 10))
    ...
    def abrir_modal_salvar_preset(self):
        self.abrir_modal_input("Salvar Preset", "Digite um nome para este preset:", self.salvar_preset_atual)

    def salvar_preset_atual(self, nome):
    ...
    def confirmar_exclusao_preset(self, nome):
        self.abrir_modal_confirmacao("Excluir Preset", f"Deseja excluir o preset '{nome}'?", lambda: self.excluir_preset(nome))
    ...
    def renomear_preset_modal(self, nome):
        self.abrir_modal_input("Renomear Preset", f"Novo nome para '{nome}':", lambda novo: self.renomear_preset(nome, novo), valor_inicial=nome)

    def renomear_preset(self, velho, novo):
        if velho in self.presets:
            self.presets[novo] = self.presets.pop(velho)
            self.salvar_presets()
            self.atualizar_lista_presets_ui()
